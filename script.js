...
window.addEventListener('load', () => {
    ...
    // Kick off the motion after the browser paints the first frame
    requestAnimationFrame(() => {
        loadingOverlay.classList.add('intro-animate');
    });

    const fadeDelay = 2200;

    setTimeout(() => {
        loadingOverlay.classList.add('intro-fade');
        document.body.classList.add('loaded');
        document.body.classList.remove('intro-visible');
    }, fadeDelay);

    loadingOverlay.addEventListener('transitionend', (event) => {
        if (event.propertyName === 'opacity' && loadingOverlay.classList.contains('intro-fade')) {
